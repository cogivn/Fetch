...
open class RxFetchImpl(
    ...
    override fun delete(ids: List<Int>): Convertible<List<Download>> {
        return synchronized(lock) {
            ...
            Flowable.just(ids)
                ...
                .flatMap { ids ->
                    throwExceptionIfClosed()
                    val downloads = fetchHandler.delete(ids, false)
                    uiHandler.post {
                        downloads.forEach {
    ...
    override fun deleteGroup(id: Int): Convertible<List<Download>> {
        return synchronized(lock) {
            ...
            Flowable.just(id)
                ...
                .flatMap { id ->
                    throwExceptionIfClosed()
                    val downloads = fetchHandler.deleteGroup(id, false)
                    uiHandler.post {
                        downloads.forEach {
                            logger.d("Deleted download $it")
                            listenerCoordinator.mainListener.onDeleted(it)
    ...
    override fun deleteAll(): Convertible<List<Download>> {
        return synchronized(lock) {
            ...
            Flowable.just(Any())
                ...
                .flatMap {
                    throwExceptionIfClosed()
                    val downloads = fetchHandler.deleteAll(false)
                    uiHandler.post {
                        downloads.forEach { download ->
    ...
    override fun deleteAllWithStatus(status: Status): Convertible<List<Download>> {
        return synchronized(lock) {
            ...
            Flowable.just(status)
                ...
                .flatMap {
                    throwExceptionIfClosed()
                    val downloads = fetchHandler.deleteAllWithStatus(it, false)
                    uiHandler.post {
                        downloads.forEach { download ->
                            logger.d("Deleted download $download")
                            listenerCoordinator.mainListener.onDeleted(download)
    ...
    override fun deleteAllInGroupWithStatus(
        ...
        return synchronized(lock) {
            ...
            Flowable.just(Pair(id, statuses))
                ...
                .flatMap {
                    throwExceptionIfClosed()
                    val downloads = fetchHandler.deleteAllInGroupWithStatus(
        ...
        return synchronized(lock) {
            ...
            Flowable.just(Pair(requests, fromServer))
                ...
                .flatMap {
                    ...
                    Flowable.just(
                        Pair(
                            results as List<Pair<Request, Long>>,
                            results2 as List<Pair<Request, Error>>
                        )
                    )
                }
                .observeOn(uiScheduler)
    ...
    ): Convertible<Downloader.Response> {
        return synchronized(lock) {
            throwExceptionIfClosed()
        ...
        synchronized(lock) {
            ...
            handlerWrapper.post {
                try {
                    ...
                    logger.e(
                        "exception occurred whiles shutting down RxFetch with namespace:$namespace",
                        e
                    )
                }
            }
